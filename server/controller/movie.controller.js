...
module.exports.create = async (req, res) => {
    ...
    try {
        ...
        if (isExistMovie) {
            return res.status(400).send({ message: "Movie is already posted or created" });
        }

        await movieModel.create({ ...req.body });
        return res.status(201).send({ message: "Movie posted successfully" });
    } catch (error) {
...
module.exports.deleteMovie = async (req, res) => {
  const movieId = req.params.movieId; // ✅ Correct param name
